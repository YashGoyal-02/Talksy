...
function Profile() {
    ...
    let image = useRef();
    let [saving,setSaving] = useState(false); // integrating loading..

    const handleImage = (e) => {
        let file = e.target.files[0] // send file to the backend.
        setBackendImage(file)
        setFrontendImage(URL.createObjectURL(file)) // give the url of frontend image.
    }

    const handleProfile = async (e) => { // updating the profile
        e.preventDefault() // refresh na ho.
        setSaving(true);
        try {
            let formData = new FormData() // sending the data in the form of formdata
  ...
  return (
    <div className='w-full h-[100vh] bg-slate-200 flex flex-col justify-center items-center gap-[20px]'>
      ...
      <div className= 'bg-white border-4 rounded-full border-[#20c7ff] shadow-gray-400 shadow-lg relative' onClick={()=>image.current.click()}>
        <div className='w-[200px] h-[200px] rounded-full overflow-hidden flex justify-center items-center cursor-pointer'>
            <img src={frontendImage} alt="" className='h-[100%]'/>
            </div>
            <div className='absolute bottom-4 right-4 text-gray-700 w-[35px] h-[35px] rounded-full bg-[#20c7ff] flex justify-center items-center shadow-gray-400 shadow-lg'>
      ...
      <form className='w-[95%] max-w-[500px] flex flex-col gap-[20px] items-center justify-center' onSubmit={handleProfile}>
        ...
        <button className='px-[20px] py-[10px] bg-[#20c7ff] rounded-2xl  shadow-gray-400 shadow-lg text-[20px] w-[200px] mt-[20px] font-semibold hover:shadow-inner text-gray-600' disabled = {saving}>{saving?"saving...":"Save Profile"}</button>
